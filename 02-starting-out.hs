...
splitAtIndex k l = ([l !! i | i <- [0..k]], [l !! i | i <- [(k + 1)..(length l - 1)]])
...
insertElem x k l = [l !! i | i <- [0..(k-1)]] ++ [x] ++ [l !! i | i <- [k..(length l - 1)]]

-- Rotate list l n places left.
-- For example, "rotate 2 [1,2,3,4,5]" gives [3,4,5,1,2]
rotate n l = [l !! i | i <- [(length l - n)..(length l - 1)]] ++ [l !! i | i <- [0..(length l - 1 - n)]]
